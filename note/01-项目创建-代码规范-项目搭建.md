...
创建项目，取名 *react-ts-music*

## 1.两种方案

方案一：使用脚手架，创建一个普通 react 项目，自行配置 TS，有以下弊端。

- React 的配置默认隐藏，需要使用 eject 弹出配置，该操作不可逆。
- 配置起来有难度，且比较麻烦，最好配置 tsc 和 babel 两种编译方式，分别负责类型检测和编译时代码转换。
...
2.创建并配置 `.prettierrc` 或 `.prettierrc.json` ⽂件；与 `.editorconfig` 的配置风格保持统一。
...
为 `.eslintrc.js` 和 `craco.config.js` 上方添加注释，消除 *eslint* 针对打包工具配置文件的警告。
...
> - 如果使用箭头函数作为函数组件，推荐显式的为组件加上名称，如 `Discover.displayName = 'Discover'`，否则 *eslint* 会报错
>
...
为 React 的 tsx 组件设置一个代码片段模板，放入编辑工具中。
...
:four: 每次在使用 `useSelector` 时，都要导入 state 的类型，似乎还是有点麻烦。

封装 `useSelector` 函数，使得封装后的函数，能够自动推导 state 类型。

从 *react-redux* 中导入 `TypedUseSelectorHook` 接口，它是一个函数调用签名。使用它进行封装。[参考文档](https://cn.redux.js.org/tutorials/typescript-quick-start#%E5%AE%9A%E4%B9%89-hooks-%E7%B1%BB%E5%9E%8B)
...
6.在 `store/index.ts` 中

封装一个自己的 `useDispatch`（非必须）。

也可以封装自己的 `shallEqual`（非必须）.
...
const foo: IFnCall<number> = function (fn, age) {
  //  return 'aaa' // 报错，无法推断返回的是 string 类型。因为无法缺点 fn 的返回值类型。
  return fn(111)
}
